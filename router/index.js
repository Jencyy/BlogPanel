...
const { isUser, isAdmin } = require('../controllers/middleware/role'); // Importing the role-based middleware
...
// Auth routes
router.get('/signUpForm',isAuth, authController.signUpFormCon);
router.post('/register', upload.single('profilePic'), authController.signUpCon);
router.get('/logInForm', authController.logInFormCon);
router.post('/login',passport.authenticate('local', { failureRedirect: '/logInForm' , successRedirect:'/'}))
