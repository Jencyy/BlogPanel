...
const verifyEmail = async (req, res) => {
    ...
    if (!user) {
        ...
        // for otp
         userOtp = otpGenerator.generate(1, { upperCaseAlphabets: false, lowerCaseAlphabets: false, specialChars: false });
        console.log(' variableotp',userOtp)
        res.redirect(`/verifyOtpForm/${user._id}`)

        // for link
        let link =`http://localhost:5725/resetPassForm/${user.id}`;
        console.log(link);
    ...
    if (newPassword === confirmPassword) {
        bcrypt.hash(newPassword, saltRounds, async (err, hash) => {
            if (err) {
                ...
                const updatedPass = await UserModel.findByIdAndUpdate(id, { password: hash });
                res.redirect('/logInForm');
            }
        });
    } else {    
